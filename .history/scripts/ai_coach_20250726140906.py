...
def main():
    ...
    try:
        ...
        print(f"\n\033[1mCoach's Win Probability Estimate:\033[0m {win_prob * 100:.2f}%")
    except Exception as e:
        print(f"[❌] Win probability prediction failed: {e}")
    print(f"\n\033[1mCoach's Advice:\033[0m {advice}")
    if scold:
        print(f"\033[91mCoach's Scolding:\033[0m {scold}")
    if motivational:
        print(f"\033[94mCoach's Motivation:\033[0m {motivational}")
    print("\n\033[1mUnique Suggestions:\033[0m")
    run_rate = runs / overs if overs > 0 else 0
    ...
    print("\n\033[1mHistorical Match Insight:\033[0m")
    ...
    while True:
        ...
        if simulate != "yes":
            break
        print("\n--- Scenario Simulation ---")
        print("Options: 1) Lose a wicket  2) Score a boundary  3) Bowl a maiden  4) Custom  5) Add 10 runs  6) Lose 2 wickets")
        ...
        print("👉 Best Bowler:", suggest_bowler(pitch, run_rate, required_rate))
        print("👉 Fielding Strategy:", suggest_field(run_rate, required_rate))
        print("\n[Scenario] Historical Match Insight:")
        print(find_similar_match(runs, wickets, overs, target))
